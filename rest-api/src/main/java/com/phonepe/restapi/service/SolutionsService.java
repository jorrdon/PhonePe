...
public class SolutionsService {
    ...
    public List<Solution> getUserSolutions(String userId, String status) {
        ...
        List<Solution> response = solutionList.stream().filter(x -> x.getStatus().equalsIgnoreCase(status)).collect(Collectors.toList());
        return response;
    }
    ...
    private void addProblemsToSolvedList(Solution solution) {
        ...
        problemsData.setNumberOfPeopleAttempted(getNumberOfPeopleAttempted(solution.getProblemId()));
        solutionList.add(solution);
        userProblemSolvedData.put(util.getProblemKey(solution.getProblemId()), solutionList);
    }

    private Long getNumberOfPeopleAttempted(String problemId) {
        return 0L;
    }
    ...
    public Solution.ProblemsData getProblemsData(String problemId) {
        ...
        List<Solution> solutionList = userProblemSolvedData.get(util.getProblemKey(problemId));
        builder.numberOfPeopleAttempted(numberOfPeopleAttempted(solutionList, problemId));
